...
class ShapeNetCoarse(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        point_path = os.path.join(self.dataset_folder, model, model+'_points.npz')
        try:
            with np.load(point_path) as data:
                vol_points = data['surface_points'].astype(np.float32)
            # with np.load(coarse_point_path) as data:
                coarse_vol_points = data['coarse_surface_points'].astype(np.float32)
